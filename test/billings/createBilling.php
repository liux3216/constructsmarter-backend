...
//-------------------------------------------------
$existing = $db->one(
    "SELECT `id` FROM `billings` WHERE `billingNumber` = ? LIMIT 1;",
    [$billingNumber],
    __FILE__, __LINE__
);
if($existing){
...
try {
    ...
    $db->exec(
        "INSERT INTO `billings` (
            ...
            `billed`,
            `creatorId`
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), 'submitted', 'no', ?);",
        [$billingNumber, $projectId, $contactId, $approverId, $fromDate, $toDate, $amount, $billable, $notes, $userId, $userId],
        __FILE__, __LINE__
    );
    ...
    exit(json_encode(["msg" => "Failed to create billing."]));
}
//-------------------------------------------------
exit(json_encode([
    "id" => $id
