...
if (!$fileIds) {
    jsonResponse(422, ["msg" => "No file ids provided."]);
}

$placeholders = implode(",", array_fill(0, count($fileIds), "?"));
$rows = $db->all(
    "SELECT `id`, `name`, `type` FROM `fileInfo` WHERE `id` IN ($placeholders);",
    $fileIds,
    __FILE__,
    __LINE__
);
...
$tempZip = tempnam(sys_get_temp_dir(), "purchase-files-");
$zip = new ZipArchive();
