...
try {
    ...
    $pdfId = generatePerDiemPdf($id, null, $perDiemRow);
    if ($pdfId !== '') {
        $db->exec("UPDATE `perDiems` SET `pdfId` = ? WHERE `id` = ?;", [$pdfId, $id], __FILE__, __LINE__);
    }
    $db->commit();
} catch (Throwable $e) {
    $db->rollback();
    ...
    exit(json_encode(['msg' => 'Failed to create per diem form']));
}
