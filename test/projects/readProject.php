...
$sql = "SELECT
`p`.`id`,
CAST(`p`.`id` AS CHAR) AS `projectHashKey`,
CONCAT_WS(' - ',
...
COALESCE(CONCAT_WS(' ', `pm`.`firstName`, `pm`.`middleName`, `pm`.`lastName`), '') AS `projectManagerName`,
...
`p`.`updaterId`,
COALESCE(CONCAT_WS(' ', `statusUser`.`firstName`, `statusUser`.`middleName`, `statusUser`.`lastName`), '') AS `statusChangerName`,
`p`.`statusChangerId`,
