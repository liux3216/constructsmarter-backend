...
try{
    $id = proposalRequireString("id");
    $existing = $db->one("SELECT `id`, `status`, `void`, `pdfId` FROM `proposals` WHERE `id` = ?;", [$id], __FILE__, __LINE__);
    if(!$existing){
        proposalJsonResponse(404, ["msg" => "Proposal not found."]);
    }
    if($existing["void"] === "yes"){
        proposalJsonResponse(400, ["msg" => "Void proposal cannot be edited."]);
    }
    ...
    $db->exec(
        "UPDATE `proposals` SET
            `proposalNumber` = ?, `proposalDate` = ?, `projectId` = ?, `requesterId` = ?, `approverId` = ?, `department` = ?,
            `notes` = ?, `subtotal` = ?, `tax` = ?, `discount` = ?, `total` = ?, `data` = ?, `status` = 'Re-Submitted',
            `submitterId` = ?, `submitTime` = NOW(), `updaterId` = ?
        WHERE `id` = ?;",
        [$proposalNumber, $proposalDate, $projectId, $requesterId, $approverId, $department, $notes, $subtotal, $tax, $discount, $total, $data, $userId, $userId, $id],
        __FILE__,
        __LINE__
    );
    $proposal = $db->one(
    ...
    $proposal["data"] = $lines;
    $pdfId = generateProposalPdf((string)$id, $proposalNumber, (string)$existing["pdfId"], $proposal);
